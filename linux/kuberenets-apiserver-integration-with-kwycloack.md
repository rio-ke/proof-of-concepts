...
extraEnv: |
  ...
  - name: "KEYCLOAK_FRONTEND_URL"
    value: 'https://identity.tiscout.co/auth'
  ...
  - alert: keycloak-IngressHigh5xxRate
    ...
    expr: |
      (
        sum(
          rate(
            nginx_ingress_controller_response_duration_seconds_count{exported_namespace="identity",ingress="identity",status=~"5[0-9]{2}"}[1m]
          )
        )
        /
        sum(
          rate(
            nginx_ingress_controller_response_duration_seconds_count{exported_namespace="identity",ingress="identity"}[1m]
          )
        )
      ) * 100 > 1
    for: 5m
...
helm install keycloak codecentric/keycloak -n keycloak -f values.yaml
```
