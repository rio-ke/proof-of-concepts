...
def gpgDecryption(combineOfKeyFile, passphrase=None, nameOfDecryoptionFile):
    gpg = gnupg.GPG(gnupghome="/tmp", gpgbinary='/opt/python/gpg')
    keyLoader = open(combineOfKeyFile).read()
    gpg.import_keys(keyLoader)
    output = removeGpgExtension(nameOfDecryoptionFile)
    print(output)
    status = gpg.decrypt_file(nameOfDecryoptionFile,
...
def uploadfile(bucketname, staticDir, key, tagname, tagValue):
    ...
    s3.meta.client.upload_file(tempPath, bucketname, s3Path, ExtraArgs={
...
def lambda_handler(event, context):
    ...
    if (checkDecryptionStatus(fileName)):
        status = gpgDecryption(downloadSecretKeyFromS3(
            KEY_BUKCET, agentName), downloadfile(eventBucket, fileName))
        ...
        if (descryptionState['decrytionState']):
            uploadfile(eventBucket, gpgProcessStateLocation,
                       reformationFileName, "status", "Decrypted")
            deleteS3Object(eventBucket, fileName)
        else:
            failedUploadFile(eventBucket, gpgFailedStateLocation,
                             reformationFileName, "status", "Decryption failed")
            deleteS3Object(eventBucket, fileName)
    else:
