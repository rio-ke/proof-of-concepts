...
resource "datadog_monitor" "bar" {
  ...
  message              = <<-EOT
    ...
    EOT
  name                 = "${var.failStepFunctionName}-failed"
  new_group_delay      = 0
  new_host_delay       = 300
  no_data_timeframe    = 0
  notify_audit         = false
  ...
  notify_no_data       = false
  priority             = 0
  query                = "sum(last_5m):sum:aws.states.executions_failed{statemachinearn:arn:aws:states:${var.region}:${aws_caller_identity.current.account_id}:statemachine:${var.failStepFunctionName}}.as_count() > 0"
  renotify_interval    = 0
  renotify_occurrences = 0
  require_full_window  = false
  tags                 = []
